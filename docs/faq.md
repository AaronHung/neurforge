...
```bash
NEURFORGE_LLM_TYPE=chat.completions  # use the default llm calling method
# basic openai configs, see `.env.full` if you're not familiar with these configs
NEURFORGE_LLM_MODEL=
NEURFORGE_LLM_BASE_URL=
NEURFORGE_LLM_API_KEY=
```
...
```bash
NEURFORGE_LLM_TYPE=litellm  # set the llm type as litellm
