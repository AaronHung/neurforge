...
# 常見 content-type -> reltype 對映（可擴充套件）
...
def _copy_and_fix_relations(original_slide, new_slide, new_el):
    """
    在 new_el 中查詢所有 r:embed 和 r:id，嘗試把對應的 part 從 original_slide 關聯到 new_slide，
    並替換成新的 rId。返回 True/False 表示是否成功（部分失敗不致命）。
    """
    r_ns = "http://schemas.openxmlformats.org/officeDocument/2006/relationships"

    # 查詢所有可能帶 r:embed 或 r:id 的元素（最常見：a:blip/@r:embed, c:chart/@r:id）
    # 使用通配，遍歷元素屬性，找出屬於 r namespace 的屬性
    for el in new_el.iter():
        ...
        for attr_name in list(el.attrib.keys()):
            if attr_name.startswith(f"{{{r_ns}}}"):
                ...
                # 如果已經處理過（某些模板會多次引用同一 rId），則複用對映（我們可以快取）
                # 先嚐試從 original_slide.part 獲取對應 part
                try:
                ...
                # fallback: 如果沒對映到 reltype，使用 image 作為最後手段（保守策略）
                if reltype is None:
                    reltype = "http://schemas.openxmlformats.org/officeDocument/2006/relationships/image"

                # 在 new_slide 上建立到該 part 的關係（如果之前已建立，相同 part 會建立重複關係——可用快取最佳化）
                try:
                    new_rId = new_slide.part.relate_to(part, reltype)
                except Exception:
                    # 有時 relate_to 會拋異常（例如 part 型別不支援），嘗試用 image reltype 作為降級
                    try:
...
def duplicate_slide(prs, slide):
    ...
    for shape in slide.shapes:
        ...
        # 先修復關係（圖片/chart/ole 等）——使用 original slide 作為資源源
        _copy_and_fix_relations(slide, new_slide, new_el)

        # 將 new_el 插入 new_slide 的 spTree，插入到 extLst 之前（與之前一致）
